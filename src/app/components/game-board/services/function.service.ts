...
export class FunctionService {
 ...
 public prepareBoard(cellX: number, cellY: number): boolean[][] {
    const board = [];
  ...
  public drawBorders(ctx: any, cellX: number, cellY: number, size: number, width: number, height: number):void {
    for( let i = 0; i < cellX ; i++ ) {
        ctx.beginPath();
  ...
  public drawAll(ctx: any, cellX: number, cellY: number, size: number, width: number, height: number, colors: string[], board: []): void {
    this._reset(ctx, width, height)
    this.drawBoard(ctx, colors, cellX, cellY, size, board)
    this.drawBorders(ctx, cellX, cellY, size, width, height)
  }
  ...
  public drawBoard(ctx: any, colors: string[], size: number , cellX: number, cellY: number, board: []): void {
    ctx.fillStyle = colors[Math.floor(Math.random() * colors.length)]

    for( let i = 0; i < cellX!; i++ ) {
      for( let j = 0; j < cellY!; j++ ) {
        if(!this.isAlive(i, j, cellX!, cellY!, board)) {
  ...
  public isAlive(x: number, y: number, cellX: number, cellY: number, board: []): number {
  ...
  public countNeighbours(x: number, y: number, cellX: number, cellY: number, board: []): number {
    let count = 0
    for( let i of [-1, 0, 1]) {
      for( let j of [-1, 0, 1]) {
        if(!(i === 0 && j === 0)) {
          count += this.isAlive( x + i, y + j, cellX, cellY, board)
        }
      }
    }
    return count
  ...
  public setNewGeneration(cellX: number, cellY: number, board: []): boolean[][] {

    const cBoard = this.prepareBoard(cellX, cellY)

    for( let i = 0; i < cellX; i++ ) {
      for( let j = 0; j < cellY; j++ ) {
        //if the cell is dead but has three neighbours, it becomes alive (true)
        if(!this.isAlive(i, j, cellX, cellY, board)) {
          if(this.countNeighbours(i, j,cellX, cellY, board) === 3 ) {
            cBoard[i][j] = true
          }
        } else {
          //if the cell is already alive and it has 2 or 3 neighbouring cells , then it stays alive (true)
          const count = this.countNeighbours(i, j, cellX, cellY, board)
          if(count == 2 || count == 3) {
            cBoard[i][j] = true
          }
        }
